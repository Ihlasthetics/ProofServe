...
Purpose: obtain the signed RP context and public, provider- and request-bound IDKit settings
...
signs only the configured World action, canonicalizes the nonce returned by that
server-side SDK signing call, and derives `signal` as
`proofserve:provider:<providerId>:nonce:<canonicalNonce>`. The client cannot
choose the action, signal, nonce, app id, RP id, environment, or key.
...
Before returning the response, the API persists the canonical lowercase nonce,
the provider id, the signed context's `expires_at`, and an unconsumed state. The
signature and other RP context fields are returned to the client but are not
persisted. If issuance cannot be recorded, the API does not return the context.
...
```json
{
  ...
  "signal": "proofserve:provider:provider_example_unverified:nonce:0x1111111111111111111111111111111111111111111111111111111111111111",
  "environment": "sandbox",
  "rp_context": {
...
Before returning the response, the API transactionally locks the provider and
requires it to be unverified or expired. It persists the canonical nonce,
provider id, server issuance time, signed expiration, unconsumed state, and the
provider's verification epoch (the verifiedAt/expiresAt pair, or null/null before
first verification). The RP signature is not persisted.
...
```json
{
  ...
  "responses": [
    {
      "identifier": "selfie",
      "signal_hash": "0x00d3501b0d89c47bccb3c0200172241958bfeb5c80d87f689506aa94390c36ad",
...
Before contacting World, the server requires the canonical nonce to identify an
unconsumed context issued by this API for the same provider, requires the server
clock to be earlier than that context's signed expiration, and requires the
provider's verification epoch to exactly match the epoch captured at issuance.
The provider must still be unverified or expired. An unissued, stale-epoch,
wrong-provider, or expired nonce returns WORLD_PROOF_INVALID. A consumed nonce
returns WORLD_PROOF_REPLAYED. The server then checks the configured action and
environment, independently canonicalizes the submitted nonce, derives
`proofserve:provider:<providerId>:nonce:<canonicalNonce>`, and compares
`signal_hash` with that signal's `hashSignal` value from the pinned official SDK.
The request's `nonce` is the non-secret, request-specific value generated by the
server-side IDKit 4.2.4 `signRequest` call and included in the signed RP context.
IDKit returns that nonce with the proof. Legacy 3.0 does not otherwise bind this
separate nonce into its cryptographic proof inputs, so including the canonical
nonce in the signal prevents an old proof from being paired with a newly issued
context. The server forwards the complete validated result without field
...
At context issuance, the server retains only the canonical lowercase RP nonce,
provider id, issuance time, signed-context expiration, verification epoch, and
consumption state. After successful
upstream verification, it also retains the canonical decimal 256-bit nullifier
and consumption timestamp. Equivalent decimal
or hexadecimal nullifier forms, hex digit/prefix casing, and leading zeros resolve
to the same nullifier key. The nonce is a non-secret request identifier; the raw
IDKit result, proof, signal, RP signature, signing key, integrity bundle, biometric
data, credentials, and upstream response are not persisted or logged.

The nullifier is permanently bound to the first provider that successfully uses
it for the configured action. A different provider can never claim that nullifier.
Each issued RP nonce is retained permanently and can be consumed once, so submitting
the same old request/proof again returns WORLD_PROOF_REPLAYED before another World
verification call. Once the provider's previous verification has expired, the
same provider may renew with the same stable nullifier only when World successfully
verifies a fresh proof bound to a context-specific signal issued after that expiry.
A spare or racing context from an earlier epoch cannot renew a later verification
even when its signed lifetime has not ended.

In production, issuance locks and reads the provider before storing its epoch.
Verification locks and rechecks the provider and issued-context row after the
pinned World call. Context consumption, nullifier ownership, and the provider update
share one PostgreSQL transaction. Database uniqueness permits only one consumer,
and a failed provider write rolls back consumption and newly created ownership.
Providers, services, verification metadata,
nullifier ownership, and issued-context consumption records persist after migration
002_registry and across API restarts; startup fails closed if the migration is
absent. Test-only in-memory repositories mirror these rules for the process lifetime
and are not production storage.
