...
provider signal. The request's `nonce` is the non-secret, request-specific value
generated by the server-side IDKit 4.2.4 `signRequest` call and included in the
signed RP context. IDKit returns that nonce with the proof. The server forwards
the complete validated result without field remapping to the fixed endpoint
...
verifies a context issued after that expiry. A spare or racing context from an
earlier epoch cannot renew a later verification even when its signed lifetime has
not ended.
